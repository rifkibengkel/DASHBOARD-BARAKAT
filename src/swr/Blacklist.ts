...
export function useSWRBlackList({ filter, pagination }: { filter: Filter; pagination: Pagination }) {
    const paramQuery = new URLSearchParams();
    appendParams(paramQuery, filter);
    appendParams(paramQuery, pagination);

    const getKeys = `/api/blacklist/list?${paramQuery.toString()}`;
    const { data, isLoading, error } = useSWR([getKeys], {
        keepPreviousData: true,
        revalidateOnFocus: false,
        shouldRetryOnError: false,
    }) as { data: PaginatedResponse<UsersList>; isLoading: boolean; error: unknown };
