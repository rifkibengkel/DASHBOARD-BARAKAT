...
function HeaderConsumerDataComponent(props: HeaderProps) {
  ...
  const exportData = (type: string) => {
    ...
    const exportUrl = `/api/consumer-data/export?${
      type ? `type=${type}&` : ""
  ...
  return (
    <Stack
      ...
      gap={2}
    >
      <Typography variant="h4">Consumer Data</Typography>
      <Stack
        flexDirection={"row"}
        justifyContent={"space-between"}
        alignItems={"center"}
        ...
        >
          {pageRole.m_insert === 1 && (
            <>
              ...
              <Button
                variant="outlined"
                onClick={() => exportData("transaction")}
          ...
          <Button
            variant="contained"
            onClick={() =>
              showModal("consumer-data-filter", { id: "", type: "filter" })
            }
            sx={{ width: 150, flexShrink: 0 }}
...
export const HeaderConsumerData = React.memo(HeaderConsumerDataComponent);
