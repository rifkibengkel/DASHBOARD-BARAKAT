...
interface TableRegistrationSummaryProps {
    headers: string[];
    rows: TableRowData[];
    loading: boolean;
    loadingRows: number;
...
const TableRegistrationSummaryComponent: React.FC<TableRegistrationSummaryProps> = ({
    headers,
    rows,
    loading,
    loadingRows,
                              ...
                              <TableRow key={idx}>
                                  {keys.map((key, hIdx) => {
                                      ...
                                      return (
                                          <TableCell
                                              key={hIdx}
                                              sx={isLastRow ? { fontWeight: 'bold', py: 1.5 } : { py: 1.5 }}
                                          >
                                              {cellValue ?? '-'}
                                          </TableCell>
                                      );
...
export const TableRegistrationSummary = React.memo(TableRegistrationSummaryComponent);
