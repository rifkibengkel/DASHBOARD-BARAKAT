...
function HeaderEntriesComponent(props: HeaderProps) {
  ...
  const exportData = (type: string) => {
    ...
    const exportUrl = `/api/entries/export?${type ? `type=${type}&` : ""}${
  ...
  return (
    <Stack
      ...
      gap={2}
    >
      <Typography variant="h4">Entries Approved</Typography>
      <Stack
        flexDirection={"row"}
        justifyContent={"space-between"}
        alignItems={"center"}
      ...
      >
        <div style={{ flex: 1 }}>
          <ChipFilter excludeFilters={["isValid", "isApprovedAdmin"]} />
        </div>
        <Stack
          flexDirection={"row"}
          justifyContent={"end"}
