...
function ModalRejectComponent({ invalidReasonList }: ModalRejectProps) {
  ...
  const modal = modals["winner-reject"];
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await fetch(`/api/winner/reject?id=${modal.id}`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Failed to reject Winner");
      }
      ...
      setError(
        error instanceof Error ? error.message : "Failed to reject Winner"
      );
      notify({
        ...
        <Grid container spacing={2}>
          ...
          <Grid size={{ xs: 12 }}>
            <Typography variant="body2" sx={{ mb: 2 }}>
              Please select a reason for rejecting this Winner:
            </Typography>
            <SelectDefault
              fullWidth
