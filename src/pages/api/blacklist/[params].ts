...
import { deleteBlackList, getBlackList, getBlackListById, insertBlackList, updateBlackList } from './_model';
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === 'GET') {
        if (req.query.params === 'list') {
            ...
            const { list, currentPage, dataPerPage, totalData, totalPage } = await getBlackList({
                key,
                column,
                direction,
                page,
            ...
            return res
                .status(200)
                .json({ list, currentPage, dataPerPage, totalData, totalPage } as PaginatedResponse<UsersList>);
        }

        if (req.query.params === 'detail') {
            const { id } = req.query;
            const data = await getBlackListById(parseInt(id as string));
    ...
    if (req.method === 'POST') {
        if (req.query.params === 'create') {
            await insertBlackList({ userId: session.userId, blacklist: req.body });
        ...
        if (req.query.params === 'delete') {
            const { id } = req.query;
            await deleteBlackList({ id: parseInt(id as string), userId: session.userId });
